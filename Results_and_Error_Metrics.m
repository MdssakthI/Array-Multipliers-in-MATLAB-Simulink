%% Parsing Simulation output into their own variables
S0=[];S1=[];S2=[];S3=[];S4=[];S5=[];S6=[];S7=[];S8=[];S9=[];S10=[];S11=[]; %empty arrays
S12=[];S13=[];S14=[];S15=[];Result_Final=[];Result_dec=[];Size_Port_A=size(Port_A_DISP);
for i=1:Size_Port_A(1)-1 %parsing simulation output data from LSB and MSB product bit
    S0=[S0 out.Sum0(1,i)];
    S1=[S1 out.Sum1(1,i)];
    S2=[S2 out.Sum2(1,i)];
    S3=[S3 out.Sum3(1,i)];
    ...
    Result_dec=[Result_dec ;bin2dec(Result)]; %Results printed and converted to double
...
%% Generation of Accurate Result for Exhaustive Simulation
Accurate_Result=[]; 
for i=0:1:N
    for j=0:255
...
%% Error Metrics Function Calling and Generating Results Matrix
